...
//		return Math.IEEEremainder(angle, Math.PI);
    ...
    public static double[] zpoly1(double[] z, double[][] C, int n) {
        ...
        int i = n - 2;
        while (i >= 0) {
            a[0] = C[i][0] + z[0] * (t = a[0]) - z[1] * a[1];
            a[1] = C[i][1] + z[0] * a[1] + z[1] * t;
            i--;
        }
        a[0] = z[0] * (t = a[0]) - z[1] * a[1];
        a[1] = z[0] * a[1] + z[1] * t;
        return a;
    }

    /* evaluate complex polynomial and derivative */
    public static double[] zpolyd1(double[] z, double[][] C, int n, double[] der) {
        ...
        while (i >= 0) {
            if (first) {
                ...
                t = b[0];
                b[0] = a[0] + z[0] * t - z[1] * b[1];
                b[1] = a[1] + z[0] * b[1] + z[1] * t;
            }
            a[0] = C[i][0] + z[0] * (t = a[0]) - z[1] * a[1];
            a[1] = C[i][1] + z[0] * a[1] + z[1] * t;
            i--;
        }
        b[0] = a[0] + z[0] * (t = b[0]) - z[1] * b[1];
        b[1] = a[1] + z[0] * b[1] + z[1] * t;
        a[0] = z[0] * (t = a[0]) - z[1] * a[1];
        a[1] = z[0] * a[1] + z[1] * t;
        der[0] = b[0];
    ...
    public static double adjlon(double lon) {
        final double SPI = 3.14159265359;
     ...
     */
    public static void sphericalRotation(double poleLat, Point2D.Double lonlat) {
        double lon = lonlat.x;
        double lat = lonlat.y;
        double sinLatPole = Math.sin(poleLat);
        double cosLatPole = Math.cos(poleLat);
        double sinLon = Math.sin(lon);
        double cosLon = Math.cos(lon);
        double sinLat = Math.sin(lat);
        double cosLat = Math.cos(lat);
        double cosLat_x_cosLon = cosLat * cosLon;
